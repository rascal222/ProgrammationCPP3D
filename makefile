...
SEGMENTATION_DIR = $(SOURCE_DIR)/segmentation
...
SRC_SEGMENTATION = $(wildcard $(SEGMENTATION_DIR)/*.cpp)
SEGMENTATION_OBJECTS = $(patsubst $(SEGMENTATION_DIR)/%.cpp,$(BUILDDIR)/%.o,$(SRC_SEGMENTATION))
...
$(SEGMENTATION_OBJECTS): $(BUILDDIR)/%.o : $(SEGMENTATION_DIR)/%.cpp
	$(CC) $(F_FLAGS) -c $< -o $@
...
tp8/SegmentationByActiveEdge: $(CORE_OBJECTS) $(SURFACE_OBJECTS) $(CURVE_OBJECTS) $(PRIMITIVE_OBJECTS) $(GLWRAPPERS_OBJECTS) $(OCTREE_OBJECTS) $(MESHING_OBJECTS) $(SEGMENTATION_OBJECTS) $(SOURCE_DIR)/tp8/mainSegmentation.cpp
	make dir
	$(CC) $(F_FLAGS) $^ -o tp8Segmentation.out $(L_FLAGS)

tp9/FusionCube: $(CORE_OBJECTS) $(SURFACE_OBJECTS) $(CURVE_OBJECTS) $(PRIMITIVE_OBJECTS) $(GLWRAPPERS_OBJECTS) $(OCTREE_OBJECTS) $(MESHING_OBJECTS) $(SEGMENTATION_OBJECTS) $(SOURCE_DIR)/tp9/mainCubeFusionTest.cpp
	make dir
	$(CC) $(F_FLAGS) $^ -o tp9FusionCube.out $(L_FLAGS)

tp9/Decimation: $(CORE_OBJECTS) $(SURFACE_OBJECTS) $(CURVE_OBJECTS) $(PRIMITIVE_OBJECTS) $(GLWRAPPERS_OBJECTS) $(OCTREE_OBJECTS) $(MESHING_OBJECTS) $(SEGMENTATION_OBJECTS) $(SOURCE_DIR)/tp9/mainDecimation.cpp
	make dir
	$(CC) $(F_FLAGS) $^ -o tp9Decimation.out $(L_FLAGS)